...
<!--         <br /><input type="submit" class="btn btn-default" role="button"  value="结束训练" onclick ="Warn('<?php echo SITE_URL;?>/index.php?r=train/over&id=<?php echo $v->train_id; ?>&model=<?php echo $v->train_model;?>&name=<?php echo $v->train_name;?>');" />        -->
<script type="text/javascript">
 var startTime = '<?php date_default_timezone_set("Asia/Shanghai");
 echo date("Y-m-d H:i:s");?>';  
 document.getElementById('RecordHomeworkPenetration_start_time').value = startTime;    
...
if(sflag == '1'){
    var answer = '<?php foreach ($recordInfos as $v)
        echo $v->answer;?>'
    document.getElementById('RecordHomeworkPenetration_answer').value = answer;
}
</script>